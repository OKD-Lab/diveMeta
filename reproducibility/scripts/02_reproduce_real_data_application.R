...
lib <- file.path(root, "review_lib")
.libPaths(c(lib, .libPaths()))

data_dir <- file.path(root, "data")
out_dir  <- file.path(root, "review_outputs", "real_data")
dir.create(out_dir, recursive = TRUE, showWarnings = FALSE)

library(diveMeta, lib.loc = lib)
library(metamedian, lib.loc = lib)
library(metafor)
library(readr)
library(tibble)
library(dplyr)
...
# Keep only studies with medians and sample sizes in both arms
dat_dive <- subset(
  dat_dive,
...
# study-level weights for Table 3
wi_dive <- dat_dive$n_g1 + dat_dive$n_g2
wtilde_dive <- wi_dive / sum(wi_dive)

## =========================================
## QE--RE (same reviewer CSV naming, internal
## temporary renaming only for metamedian())
## =========================================
...
# Keep only studies with full S2 information in both arms
dat_qe <- subset(
  dat_qe,
...
# metamedian expects its own naming convention; create a temporary copy
dat_qe_mm <- dat_qe |>
  dplyr::transmute(
    study_id = study_id,
    n.g1     = n_g1,
...
readr::write_csv(table3_review, file.path(out_dir, "Table3_real_data_inputs_review.csv"))
...
readr::write_csv(table4_review, file.path(out_dir, "Table4_real_data_summary_review.csv"))

## ==========================
## Simple reviewer text output
## ==========================
